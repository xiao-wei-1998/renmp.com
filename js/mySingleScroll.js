(function($) {
    ...
    function nextMove(obj, step) {
        obj.find("ul").animate({ marginLeft: -step }, 300, 'swing', function() {
    ...
    function preMove(obj, step) {
        ...
        ul.animate({ marginLeft: 0 }, 300, 'swing', function() { ul.css("margin-left", 0); });
    }

    function downMove(obj, step) {
        obj.find("ul").animate({ marginTop: -step }, 300, 'swing', function() {
            $(this).find("li").slice(0, 1).appendTo($(this));
    ...
    function upMove(obj, step) {
        ...
        ul.animate({ marginTop: 0 }, 300, 'swing', function() { ul.css("margin-top", 0); });
    }
    var methods = {
        init: function(options) {
            var defaults = { speed: 3000, direction: 'horizantal' };
            ...
            return this.each(function(i) {
                ...
                if (isMove) {
                    timers[i] = setInterval(function() { move(_this, sh); }, speed);
                    _this.hover(function() { clearInterval(timers[i]); }, function() { timers[i] = setInterval(function() { move(_this, sh); }, speed); });
                    pre.click(function() { lmove(_this, sh); });
                    next.click(function() { rmove(_this, sh); });
                }
            });
        },
        destroy: function() {
    ...
    $.fn.mySingleScroll = function(options) {
        ...
        if (methods[method]) { method = methods[method]; } else if (typeof method == 'object' || !method) { method = methods.init; } else { $.error('error'); return this; }
